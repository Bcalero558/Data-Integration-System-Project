import psycopg2
from src import visualize, reader, validate ,database
...
def main():
    ...
    gm_data = gm_reading.read_csv("data\\processed\\clean_data.csv",format_str)

    gm_transfer = database.DatabaseOperations(gm_data)
    gm_db = gm_transfer.connect_to_database()
    gm_transfer.create_table(gm_db)
    gm_transfer.insert_data(gm_db)

    query = gm_transfer.query(gm_db)
    visualize.DataWindow(query)
